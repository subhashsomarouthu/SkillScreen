...
export default function ModernInterviewScreen({ participantName, fromToken = false }: ModernInterviewScreenProps) {
  ...
  // Mode state
  const [interviewMode, setInterviewMode] = useState<'video' | 'audio' | 'chat'>('video');
  ...
  useEffect(() => {
    ...
    const startInterviewAndGetFirstQuestion = async () => {
      ...
      try {
        ...
        // Fetch interview details first to get the mode
        try {
          const detailsResponse = await apiClient.getInterviewDetails(interviewId);
          if (detailsResponse.success && detailsResponse.data) {
            const mode = detailsResponse.data.mode || 'video';
            console.log('ℹ️ Interview Mode detected:', mode);
            setInterviewMode(mode as any);
            if (mode === 'chat') {
              setIsVideoOn(false);
              setIsMuted(true);
            }
          ...
          console.warn('Failed to fetch interview details, defaulting to video mode', e);
        }
        ...
        if (data.success) {
          ...
          // Play audio automatically (Skip for Chat)
          if (data.data.audio_url && interviewMode !== 'chat') { // Check mode explicitly though state might not update instantly in same tick, but effect rerun will handle? No, better use local var if needed, but here simple check is ok as audio auto-play is secondary
  ...
  useEffect(() => {
    ...
    // 5. NOT in Chat mode
    if (currentQuestion && interviewStatus === 'continue' && !isRecording && !isLoadingQuestion && interviewMode !== 'chat') {
  ...
  }, [currentQuestion, interviewStatus, interviewMode]);
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      const stream = await navigator.mediaDevices.getUserMedia({
        video: interviewMode === 'video', // Only request video for video mode
        audio: true
      });
      ...
      if (interviewMode === 'audio') {
        ...
        if (MediaRecorder.isTypeSupported('audio/webm; codecs=opus')) {
          ...
          console.warn('⚠️ No specific audio mimeType found, using default');
  ...
  const submitTextResponse = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/orchestration/api/questions/next`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          interview_id: interviewId,
          session_id: currentQuestion.session_id,
          candidate_response: chatInput
        })
      });

      const data = await response.json();
      await handleNextQuestionResponse(data);
  ...
  const submitAudioAndGetNextQuestion = async (audioBlob: Blob) => {
    if (!interviewId || !currentQuestion?.session_id) return;
    setIsLoadingQuestion(true);
    try {
      console.log('📤 Submitting audio answer...');
      const formData = new FormData();
      formData.append('audio_file', audioBlob, 'answer.webm');
      formData.append('interview_id', interviewId);
      formData.append('session_id', currentQuestion.session_id);

      const response = await fetch(`${API_BASE_URL}/orchestration/api/questions/submit-audio-response`, {
        method: 'POST',
        body: formData
      });
  ...
  const handleNextQuestionResponse = async (data: any) => {
    ...
    if (data.status === 'continue') {
      ...
      if (data.audio_download_url && audioRef.current && interviewMode !== 'chat') {
      ...
      if (data.language) setCodingLanguage(data.language);
  ...
  const submitVideoAndGetNextQuestion = async (videoBlob: Blob) => {
    ...
    if (interviewMode === 'audio') {
      return submitAudioAndGetNextQuestion(videoBlob);
    }
  ...
  return (
    <div className="absolute inset-0 flex flex-col">
      {/* Interview Status Bar - Show during video/audio stage */}
      {interviewStage === 'video' && interviewMode !== 'chat' && (
          ...
          <AnimatePresence>
            {!isCodeEditorOpen ? (
              ...
              >
                {interviewMode === 'chat' ? (
                  /* Chat Mode Layout */
                  <div className="absolute inset-0 flex flex-col items-center justify-center p-6 bg-gray-900/50">
                    <div className="w-full max-w-4xl space-y-8">
                      ...
                      <div className="glass-dark rounded-2xl p-6 border border-white/10">
                        <textarea
                          ...
                          className="w-full h-48 bg-black/20 rounded-xl p-4 text-white text-lg placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-blue-500/50 resize-none"
                        />
                        <div className="flex justify-end mt-4">
                          <button
                            ...
                            className={`px-8 py-3 rounded-xl font-semibold text-white shadow-lg transition-all flex items-center gap-2
                                          ${!chatInput.trim() || isLoadingQuestion
                                ? 'bg-gray-600 cursor-not-allowed opacity-50'
                                : 'bg-blue-600 hover:bg-blue-700 hover:scale-105'
                              }`}
                          >
                            {isLoadingQuestion ? 'Sending...' : 'Submit Answer'}
                  ...
                  <div className="absolute inset-0 flex items-center justify-center p-6">
                    ...
                    <div className="relative w-full max-w-[1600px] mx-auto aspect-video">
                      ...
                      >
                        {interviewMode === 'video' ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center bg-gray-900 relative">
                            ...
                            <div className="text-center z-10">
                              <div className="w-48 h-48 rounded-full bg-gradient-to-tr from-blue-500/20 to-purple-500/20 flex items-center justify-center mx-auto mb-8 relative">
                                <div className={`absolute inset-0 rounded-full border-2 border-blue-400/30 ${isRecording ? 'animate-ping' : ''}`} />
                                <Mic className={`w-20 h-20 ${isRecording ? 'text-red-400' : 'text-blue-400'}`} />
                              </div>
                              <h3 className="text-2xl text-white font-medium mb-2">Audio Interview</h3>
                              <p className="text-white/50">
                                {isRecording ? 'Recording is active...' : 'Waiting to start...'}
                              </p>
                            </div>
                          </div>
                        )}
                        ...
                        {!isRecording && interviewMode === 'video' && (
      ...
      {/* Current Question Display - Only show during video/audio stage (Chat has its own) */}
      {currentQuestion && interviewStatus === 'continue' && interviewStage === 'video' && interviewMode !== 'chat' && (
        <motion.div
      ...
      {/* Next/Submit Button - Only show during video/audio stage (Chat has its own) */}
      {currentQuestion && isRecording && interviewStage === 'video' && interviewMode !== 'chat' && (
        <motion.div
          ...
          <button
            onClick={async () => {
              ...
              if (mediaRecorderRef.current && mediaRecorderRef.current.state !== 'inactive') {
                ...
                const mimeType = interviewMode === 'audio' ? 'audio/webm' : 'video/webm';
                ...
                await submitVideoAndGetNextQuestion(blob);
              }
            }}
            disabled={isLoadingQuestion}
            className={`px-8 py-4 rounded-2xl font-semibold text-lg shadow-2xl transition-all ${interviewStatus === 'completed'
